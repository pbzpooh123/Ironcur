...
public class TurnManager : NetworkBehaviour
{
    ...
    { 
        var players = GameManager.Instance.Players; if (players.Count == 0) 
            return;
        // 🎲 Shuffle the order
        turnOrder = new List<PlayerPawn>(players); 
        ShuffleList(turnOrder); 
        currentPlayerIndex.Value = 0; 
        Debug.Log("[TurnManager] Randomized turn order:");
        for (int i = 0; i < turnOrder.Count; i++)
        {
            Debug.Log($" Slot {i+1}: {turnOrder[i].playerName.Value}");
        } 
        StartTurn(); 
    } 
    ...
    { if (turnOrder.Count == 0) 
        ...
        if (nextIndex >= turnOrder.Count) { nextIndex = 0; turnCount.Value++; 
            Debug.Log($"[TurnManager] Completed a full cycle. TurnCount={turnCount.Value}");
            if (turnCount.Value % turnOrder.Count == 0)
            {
                roundCount.Value++; RpcUpdateRoundUI(roundCount.Value); Debug.Log($"[TurnManager] Round {roundCount.Value} completed!"); MarketManager.Instance.ProcessPayouts();
            } 
        } 
        ...
        {
            Debug.Log("Game Over! Count money and decide winner."); return;
        }
        StartTurn(); 
    }
    ...
    {
        if (turnOrder.Count == 0) return null; return turnOrder[currentPlayerIndex.Value];
    } 
    // === Utility ===
    private void ShuffleList<T>(List<T> list)
    {
        ...
        while (n > 1)
        {
            int k = rng.Next(n--); (list[n], list[k]) = (list[k], list[n]);
        }
    }
}
